...
if let currentHR = heartRate {
    let totalHR = oldHR1 + oldHR2 + oldHR3 + oldHR4 + currentHR
    let numberOfReadings = 5
    let hrAverage = totalHR / numberOfReadings
    print("avg heart rate = \(hrAverage)")
} else {
    print("Heart rate is not available.")
}

//var temp = oldHR1 + oldHR2 + oldHR3 + oldHR4 + heartRate
//var temp_avg = temp / 5
//print(temp_avg)
