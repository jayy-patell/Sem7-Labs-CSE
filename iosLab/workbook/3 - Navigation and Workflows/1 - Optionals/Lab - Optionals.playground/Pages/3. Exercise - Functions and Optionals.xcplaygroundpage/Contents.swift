...
func checkAge(_ ageString: String) -> Int? {
    if let age = Int(ageString) {
        if age > 18 {
            ...
            print("Notold enough to use our app.")
        }
        return age
    } else {
...
let validAge: String = "25"
if let age = checkAge(validAge) {
    print("Returned age: \(age)")
} else {
    print("No valid age returned.")
}
...
func itemPrice(item: String) -> Double? {
...
let itemName = "Juice"
if let price = itemPrice(item: itemName) {
    print(price)
} else {
    print("No item")
}
